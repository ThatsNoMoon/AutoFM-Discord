...
class VoiceEventListener(private val parent: AutoFM): ListenerAdapter() {
    ...
    override fun onGenericGuildVoice(event: GenericGuildVoiceEvent) {
        ...
        val members: List<Member> = when (event) {
            ...
            is GuildVoiceMoveEvent ->
                when {
                    event.channelJoined.members.contains(guild.selfMember) -> event.channelJoined.members
                    event.channelLeft.members.contains(guild.selfMember) -> event.channelLeft.members
                    else -> return
                }
            else -> return
        }
